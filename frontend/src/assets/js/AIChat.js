...
// 3. 쇼핑몰 추천하기
...
} else if (text.startsWith("shopping_")) {
    let key = text.replace("shopping_", ""); // ✅ "shopping_" 제거
    let themeMapping = {
        "mountain": "등산",
        "swimming": "물놀이",
        "surfing": "서핑",
    ...
    let theme = themeMapping[key]; // ✅ 매핑된 한글 키값 가져오기

    console.log("선택한 쇼핑몰 테마:", theme); // ✅ 디버깅용 콘솔 로그 추가

    if (theme && shoppingMallLinks[theme]) {
        selectedData["쇼핑 테마"] = theme;
        ...
        botResponse = {
            type: "bot",
            text: `✅ "${theme}" 관련 쇼핑몰 목록입니다.<br>${mallList}`,
            buttons: [
                { text: "다른 쇼핑몰 보기", action: "shopping" },
                { text: "처음으로 돌아가기", action: "restart" }
            ]
        };
    } else {
    ...
    console.log("botResponse:", botResponse); // ✅ 디버깅용 콘솔 로그 추가
}
...
return { updatedMessages }; // ✅ 올바른 위치에서 반환
}
// ✅ 챗봇 스크롤 자동 내리기 함수
