<script>
  ...
  async function onProfileUpload(event) {
    handleProfileUpload(event, async (preview, file) => {
      ...
      uploadedProfileImage = await uploadProfileImage(file); // ✅ 업로드된 URL 저장
    });
  }
    ...
    >
      ...
      <div class="mb-3 d-flex align-items-center">
        ...
        <button
          ...
          type="button"
          on:click={() =>
            checkUserId(userId, (newErrors) => (errors = newErrors))}
        >
          중복확인
        </button>
      </div>
      {#if errors.idError}
        <p style="color: red;">{errors.idError}</p>
      {/if}
      <!-- 비밀번호 -->
      <div class="mb-3">
        <input
      ...
      <div class="mb-4 d-flex align-items-center">
        ...
        <button
          ...
          type="button"
          on:click={() => {
            checkNickname(nickname, (newErrors) => (errors = newErrors));
          }}
        >
          중복확인
        </button>
      ...
      {#if errors.nickError}
        <p style="color: red;">{errors.nickError}</p>
      {/if}

      <div class="btn-group">
        <button type="button" class="cancel_btn" on:click={handleCancel}>
          취소
        </button>
